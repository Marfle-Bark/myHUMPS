...
%THIS IS THE LAST PARAGRAPH OF INTRODUCTION SECTION
The remainder of this paper is organized as follows. In Section~\ref{section:relatedwork} we review related work to medium access control. In Section~\ref{section:analysis} we discuss the results of the four different simulations: startard WiFi MAC, MAC protocol based on the SRPT algorithm, back off based on the current frame number in the stream, and lastly a dynamic back off window changed based on the traffi level in the network.  Finally, we draw the relevant conclusions in Section~\ref{section:conclusions}. 
